...
static void mmm_yummy_poop(const void* info, int* enabled, int* prev_enabled, int* render, const char* teh_name)
{
	...
	else
	{
		...
		{
			cheat_state_text("Switched to s0be1t %s", teh_name);
			*render = 1;
		}
		else if (!*prev_enabled && *enabled)
		{
...
void RenderDebug(void)
{
	...
	/* blue box */
	render->D3DBoxi(x - 2, y - 2, CHAR_WIDTH * 75 + 4, ROW_HEIGHT * 27 + 4, D3DCOLOR_XRGB(255, 0, 0), NULL);
...
D3DCOLOR RandomColor()
{
	...
	D3DCOLOR color = D3DCOLOR_XRGB(222, 38, 38);
...
void RenderMenu(void)
{
	...
	char				title[256] = "                         PROJECT KILLARK";
	...
	render->D3DBoxBorderi(left, top, MENU_WIDTH, ROW_HEIGHT + 2,
		D3DCOLOR_XRGB(152, 135, 255), D3DCOLOR_ARGB(menu_titlebar_background->alpha,
			menu_titlebar_background->red, menu_titlebar_background->green,
			menu_titlebar_background->blue));
	pD3DFont->PrintShadow((float)(left + 1), (float)(top + 1), D3DCOLOR_XRGB(152, 135, 255), &title[cp]);

	/* draw window */
	left = pPresentParam.BackBufferWidth / 2 - MENU_WIDTH / 2;
	top = pPresentParam.BackBufferHeight - MENU_HEIGHT - 1 - 20;
	render->D3DBoxBorderi(left, top, MENU_WIDTH, MENU_HEIGHT, D3DCOLOR_XRGB(152, 135, 255),
...
void RenderPedHPBar(void)
{
	...
	render->D3DBoxi(0, bottom - fontHeight + 1, (int)info->hitpoints, fontHeight - 2, D3DCOLOR_ARGB(127, 225, 39, 39), 100);
	...
	{
		render->D3DBoxi(0, bottom - (fontHeight * 2) + 1, (int)info->armor, fontHeight - 2, D3DCOLOR_ARGB(127, 211, 211, 211), 100);
...
{
	traceLastFunc("renderPlayerT4gs()");
	...
	// Exit this function and enable samp nametags, if panic key
	if (cheat_state->_generic.cheat_panic_enabled || !cheat_state->render_player_t4gs)
	{
		if (g_dwSAMP_Addr && g_SAMP)
		{
			sampPatchDisableNameTags(0);
	...
	while (iter.pos < iter.end)
	{
		...
		// check if it's farther than set.player_tags_dist
		ourPosMinusIter = ourPosition - iterPosition;
		if (ourPosMinusIter.Length() > set.player_tags_d1st)
		{
			g_playerTagInfo[iGTAID].isPastMaxDistance = true;
			continue;
		}
	...
	// start render ESP tags
	if (cheat_state->killark.killarkesp)
	{
		float h, playerBaseY;
		while (iter.pos < iter.end)
		{
			...
			D3DCOLOR    color = D3DCOLOR_ARGB(255, 152, 135, 255); //shift g esp//
			if (vh > 100.0f)
				vh = 100.0f;
			if (vh < 100.0f && vh > 90.0f)
				color = D3DCOLOR_ARGB(255, 152, 135, 255);
			if (vh < 90.0f && vh > 60.0f)
				color = D3DCOLOR_ARGB(255, 140, 80, 255);
			if (vh < 60.0f && vh > 40.0f)
				color = D3DCOLOR_ARGB(255, 170, 20, 255);
			if (vh < 40.0f && vh > 20.0f)
				color = D3DCOLOR_ARGB(255, 200, 0, 100);
			if (vh < 20.0f && vh > 0.0f)
				color = D3DCOLOR_ARGB(255, 235, 0, 0);


			render->D3DBox(g_playerTagInfo[iGTAID].tagPosition.fX + ESP_tag_player_D3DBox_pixelOffsetX,
				playerBaseY + ESP_tag_player_D3DBox_pixelOffsetY, 100.0f, 28.0f, D3DCOLOR_ARGB(150, 0, 0, 0));
			render->D3DBox(g_playerTagInfo[iGTAID].tagPosition.fX + 1.0f + ESP_tag_player_D3DBox_pixelOffsetX,
			...
			render->D3DBox(g_playerTagInfo[iGTAID].tagPosition.fX + 1.0f + ESP_tag_player_D3DBox_pixelOffsetX,
				playerBaseY + 1.0f + ESP_tag_player_D3DBox_pixelOffsetY + 18.f, ping, 8.0f, D3DCOLOR_ARGB(255, 0, 128, 128));
			...
			if (armbar > 100.0f)
				armbar = 100.0f;
			render->D3DBox(g_playerTagInfo[iGTAID].tagPosition.fX + 1.0f + ESP_tag_player_D3DBox_pixelOffsetX,
				playerBaseY + 1.0f + ESP_tag_player_D3DBox_pixelOffsetY + 10.f, va - 2.0f, 8.0f, D3DCOLOR_ARGB(170, 186, 186, 186));
			offY += 10.0f;
			...
			if (!g_Players)
			{
				...
				_snprintf_s(buf, sizeof(buf) - 1, "Armor: %d", (int)va);
				pD3DFontFixedSmall->PrintShadow(g_playerTagInfo[iGTAID].tagPosition.fX + 8.0f, playerBaseY - h + 20.0f,
					D3DCOLOR_ARGB(130, 0xFF, 0x6A, 0), buf);

				_snprintf_s(buf, sizeof(buf) - 1, "Ping: %i",
					(int)g_Players->pRemotePlayer[iSAMPID]->iPing);
				pD3DFontFixedSmall->PrintShadow(g_playerTagInfo[iGTAID].tagPosition.fX + 8.0f, playerBaseY - h + 29.0f,
					D3DCOLOR_ARGB(130, 0xFF, 0x6A, 0), buf);
			}
			else
			{
				...
				if (cheat_state->killark.haha_friends[iSAMPID])
					sprintf_s(buf, "{00FF00}%s [%d]", getPlayerName(iSAMPID), iSAMPID);


				else
					sprintf_s(buf, "%s [%d]", getPlayerName(iSAMPID), iSAMPID, g_Players->pRemotePlayer[iSAMPID]);
				pD3DFont_sampStuff->PrintShadow(g_playerTagInfo[iGTAID].tagPosition.fX - 0.0f, playerBaseY - h + -8.0f,
					samp_color_get(iSAMPID, 0xDD000000), buf);


				//if (cheat_state->wallner.haha_friends[iSAMPID])
				//{
					//h = pD3DFont_sampStuff->DrawHeight() - 9;
					//sprintf_s(buf, "WALLNER");
					//pD3DFontFixedSmall->PrintShadow(g_playerTagInfo[iGTAID].tagPosition.fX, playerBaseY - h - 27.0f,
				...
				{
					render->D3DBox(g_playerTagInfo[iGTAID].tagPosition.fX + 100.0f, playerBaseY - h + 34.0f, 50.0f, 10.0f, D3DCOLOR_ARGB(110, 0, 0, 0));
					pD3DFontFixedSmall->PrintShadow(g_playerTagInfo[iGTAID].tagPosition.fX + 100.0f, playerBaseY - h + 33.0f,
						D3DCOLOR_ARGB(130, 0xFF, 0x6A, 0), "OnFoot");
					break;
				}
				case PLAYER_STATE_DRIVER:
					render->D3DBox(g_playerTagInfo[iGTAID].tagPosition.fX + 100.0f, playerBaseY - h + 34.0f, 40.0f, 10.0f, D3DCOLOR_ARGB(110, 0, 0, 0));
					pD3DFontFixedSmall->PrintShadow(g_playerTagInfo[iGTAID].tagPosition.fX + 100.0f, playerBaseY - h + 33.0f,
						D3DCOLOR_ARGB(130, 0xFF, 0x6A, 0), "Driver");
					break;
				case PLAYER_STATE_PASSENGER:
					if (g_Players->pRemotePlayer[iSAMPID]->pPlayerData->iPassengerDriveBy)
					{
						render->D3DBox(g_playerTagInfo[iGTAID].tagPosition.fX + 100.0f, playerBaseY - h + 34.0f, 60.0f, 10.0f, D3DCOLOR_ARGB(110, 0, 0, 0));
						pD3DFontFixedSmall->PrintShadow(g_playerTagInfo[iGTAID].tagPosition.fX + 100.0f, playerBaseY - h + 33.0f,
							D3DCOLOR_ARGB(130, 0xFF, 0x6A, 0), "Drive By");
					}
					else
					{
						render->D3DBox(g_playerTagInfo[iGTAID].tagPosition.fX + 100.0f, playerBaseY - h + 34.0f, 65.0f, 10.0f, D3DCOLOR_ARGB(110, 0, 0, 0));
						pD3DFontFixedSmall->PrintShadow(g_playerTagInfo[iGTAID].tagPosition.fX + 100.0f, playerBaseY - h + 33.0f,
							D3DCOLOR_ARGB(130, 0xFF, 0x6A, 0), "Passenger");
						break;
					}
				case PLAYER_STATE_SPAWNED:
					...
					{
						render->D3DBox(g_playerTagInfo[iGTAID].tagPosition.fX + 100.0f, playerBaseY - h + 34.0f, 55.0f, 10.0f, D3DCOLOR_ARGB(110, 0, 0, 0));
						pD3DFontFixedSmall->PrintShadow(g_playerTagInfo[iGTAID].tagPosition.fX + 100.0f, playerBaseY - h + 33.0f,
							D3DCOLOR_ARGB(130, 0xFF, 0x6A, 0), "Spawned");
						break;
					}
				case PLAYER_STATE_WASTED:
					...
					else
					{
						render->D3DBox(g_playerTagInfo[iGTAID].tagPosition.fX + 100.0f, playerBaseY - h + 34.0f, 50.0f, 10.0f, D3DCOLOR_ARGB(110, 0, 0, 0));
						pD3DFontFixedSmall->PrintShadow(g_playerTagInfo[iGTAID].tagPosition.fX + 100.0f, playerBaseY - h + 33.0f,
							D3DCOLOR_ARGB(130, 0xFF, 0x6A, 0), "Wasted");
						break;
					}
				}
				...
				for (int i = 0; i < 45; i++)
				{
					if (getPlayerState(iSAMPID) == PLAYER_STATE_DRIVER)
					{
						if (weapon_list[i].id == g_Players->pRemotePlayer[iSAMPID]->pPlayerData->inCarData.byteCurrentWeapon)
						{
							_snprintf_s(bufdwa, sizeof(bufdwa) - 1, "%s", weapon_list[i].name);
							float w = pD3DFontFixedSmall->DrawLength(bufdwa);
							h = pD3DFontFixedSmall->DrawHeight() - 9;
							pD3DFontFixedSmall->PrintShadow(g_playerTagInfo[iGTAID].tagPosition.fX + 30.0f, playerBaseY + 19.0f + offY,
								D3DCOLOR_ARGB(255, 255, 139, 0), bufdwa);
							break;
						}
					}
					if (getPlayerState(iSAMPID) == PLAYER_STATE_ONFOOT)
					...
					if (getPlayerState(iSAMPID) == PLAYER_STATE_PASSENGER)
					{
						if (weapon_list[i].id == g_Players->pRemotePlayer[iSAMPID]->pPlayerData->passengerData.byteCurrentWeapon)
						{
							if (g_Players->pRemotePlayer[iSAMPID]->pPlayerData->iPassengerDriveBy == 0)
							{
								h = pD3DFont_sampStuff->DrawHeight() - 25.0f;//1
								_snprintf_s(bufdwa, sizeof(bufdwa) - 1, "%s", weapon_list[i].name);
								pD3DFontFixedSmall->PrintShadow(g_playerTagInfo[iGTAID].tagPosition.fX + 40.0f, playerBaseY + 19.0f + offY,
									D3DCOLOR_ARGB(255, 255, 139, 0), bufdwa);
								break;
							}
							if (g_Players->pRemotePlayer[iSAMPID]->pPlayerData->iPassengerDriveBy)
							{
								h = pD3DFont_sampStuff->DrawHeight() - 25.0f;//1
								_snprintf_s(bufdwa, sizeof(bufdwa) - 1, "%s (DRIVE-BY) ", weapon_list[i].name);
								pD3DFontFixedSmall->PrintShadow(g_playerTagInfo[iGTAID].tagPosition.fX + 30.0f, playerBaseY + 19.0f + offY,
									D3DCOLOR_ARGB(255, 255, 139, 0), bufdwa);
								break;
						...
						if (getPlayerState(iSAMPID) == PLAYER_STATE_NONE)
						{
							if (weapon_list[i].id == g_Players->pRemotePlayer[iSAMPID]->pPlayerData->passengerData.byteCurrentWeapon)
							{
								_snprintf_s(bufdwa, sizeof(bufdwa) - 1, "%s", weapon_list[i].name);
						...
						if (getPlayerState(iSAMPID) == PLAYER_STATE_SPAWNED)
						{
							if (weapon_list[i].id == g_Players->pRemotePlayer[iSAMPID]->pPlayerData->passengerData.byteCurrentWeapon)
							{
								_snprintf_s(bufdwa, sizeof(bufdwa) - 1, "%s", weapon_list[i].name);
						...
						if (getPlayerState(iSAMPID) == PLAYER_STATE_WASTED)
						{
							if (weapon_list[i].id == g_Players->pRemotePlayer[iSAMPID]->pPlayerData->passengerData.byteCurrentWeapon)
							{
								_snprintf_s(bufdwa, sizeof(bufdwa) - 1, "%s", weapon_list[i].name);
					...
					{
						h = pD3DFontDebugWnd->DrawHeight() - 1;
						_snprintf_s(buf, sizeof(buf) - 1, "", getPlayerName(iSAMPID), iSAMPID);
						...
						pD3DFont_sampStuff->PrintShadow(g_playerTagInfo[iGTAID].tagPosition.fX, playerBaseY - h - 8.0f, samp_color_get(iSAMPID, 0xDD000000), buf);
					}

					if (g_Players->pRemotePlayer[iSAMPID]->pPlayerData->iAFKState == 2)
					{
						_snprintf_s(bufdwa, sizeof(bufdwa) - 1, "AFK");

						float w = pD3DFontFixedSmall->DrawLength(bufdwa);
						h = pD3DFontFixedSmall->DrawHeight() + 1;

						fPlayerNameTagX = g_playerTagInfo[iGTAID].tagPosition.fX + ESP_tag_player_D3DBox_pixelOffsetX / 2 + 50.0f - pD3DFont_sampStuff->DrawLength(bufdwa) / 2;
						...
						render->D3DBox(fPlayerNameTagX + 62.0f, fAFKTextY - 10.0f, pD3DFont_sampStuff->DrawLength(bufdwa) + 2.0f, 10.0f, D3DCOLOR_ARGB(150, 0, 0, 0));
						pD3DFontFixedSmall->PrintShadow(fPlayerNameTagX + 63.0f, fAFKTextY - 12.0f, D3DCOLOR_ARGB(130, 255, 0, 0), bufdwa);
					}
					if (cheat_state->killark.killarkesp = true)
					{
						cheat_state->killark.wallner = false;		
						cheat_state->killark.jhonnyproject = false;
	...
	if (cheat_state->killark.wallner)
	{
		float h, playerBaseY;
		while (iter.pos < iter.end)
		{
			...
			g_playerTagInfo[iGTAID].tagPosition.fX -= 50.0f;
			updateScoreboardData();
			D3DCOLOR    color = D3DCOLOR_ARGB(160, 50, 160, 255); //shift g esp//
			if (vh > 100.0f)
				vh = 100.0f;
			if (vh < 100.0f && vh > 90.0f)
				color = D3DCOLOR_ARGB(160, 50, 160, 255);
			if (vh < 90.0f && vh > 60.0f)
				color = D3DCOLOR_ARGB(170, 100, 50, 255);
			if (vh < 60.0f && vh > 40.0f)
				color = D3DCOLOR_ARGB(170, 170, 20, 255);
			if (vh < 40.0f && vh > 20.0f)
				color = D3DCOLOR_ARGB(130, 200, 0, 100);
			if (vh < 20.0f && vh > 0.0f)
				color = D3DCOLOR_ARGB(200, 101, 0, 0);
				...
				for (int i = 0; i < 45; i++)
				{
					if (getPlayerState(iSAMPID) == PLAYER_STATE_DRIVER)
					{
						if (weapon_list[i].id == g_Players->pRemotePlayer[iSAMPID]->pPlayerData->inCarData.byteCurrentWeapon)
						{
							...
							h = pD3DFontFixedSmall->DrawHeight() - 9;
							pD3DFontFixedSmall->PrintShadow(g_playerTagInfo[iGTAID].tagPosition.fX + 30.0f, playerBaseY + 19.0f + offY,
								D3DCOLOR_ARGB(255, 163, 76, 0), bufdwa);
							break;
						}
					}
					if (getPlayerState(iSAMPID) == PLAYER_STATE_ONFOOT)
					...
					if (g_Players->pRemotePlayer[iSAMPID]->pPlayerData->iAFKState == 2)
					{
						...
						render->D3DBox(fPlayerNameTagX + 62.0f, fAFKTextY - 10.0f, pD3DFont_sampStuff->DrawLength(buf) + 2.0f, 10.0f, D3DCOLOR_ARGB(150, 0, 0, 0));
						pD3DFontFixedSmall->PrintShadow(fPlayerNameTagX + 63.0f, fAFKTextY - 12.0f, D3DCOLOR_ARGB(130, 255, 0, 0), buf);
					}
					if (cheat_state->killark.wallner = true)
					{
						cheat_state->killark.veipzesp = false;
						cheat_state->killark.astrong = false;
						cheat_state->killark.killarkesp = false;
						cheat_state->killark.jhonnyproject = false;
					}
				}
			}
	...
	if (cheat_state->killark.jhonnyproject)
	{
		float h, playerBaseY;
		while (iter.pos < iter.end)
		{
			...
			g_playerTagInfo[iGTAID].tagPosition.fX -= 50.0f;
			updateScoreboardData();
			/*D3DCOLOR	color = D3DCOLOR_ARGB(130, 0, 255, 0); // 2.9.3
			if (vh > 100.0f)
			vh = 100.0f;
			if (vh < 100.0f && vh > 60.0f)
			color = D3DCOLOR_ARGB(120, 0, 255, 0);
			if (vh < 60.0f && vh > 20.0f)
			color = D3DCOLOR_ARGB(110, 255, 255, 0);
			if (vh < 20.0f && vh > 0.0f)
			...
			render->D3DBox(g_playerTagInfo[iGTAID].tagPosition.fX + ESP_tag_player_D3DBox_pixelOffsetX,
				playerBaseY + ESP_tag_player_D3DBox_pixelOffsetY, 100.0f, 28.0f, D3DCOLOR_ARGB(111, 0, 0, 0));
			render->D3DBox(g_playerTagInfo[iGTAID].tagPosition.fX + 1.0f + ESP_tag_player_D3DBox_pixelOffsetX,
				playerBaseY + 1.0f + ESP_tag_player_D3DBox_pixelOffsetY, vh - 2.0f, 8.0f, color);
			...
			if (armbar > 100.0f)
				armbar = 100.0f;
			render->D3DBox(g_playerTagInfo[iGTAID].tagPosition.fX + 1.0f + ESP_tag_player_D3DBox_pixelOffsetX,
				playerBaseY + 1.0f + ESP_tag_player_D3DBox_pixelOffsetY + 10.f, va - 2.0f, 8.0f, D3DCOLOR_ARGB(120, 186, 186, 186));
			offY += 10.0f;
				...
				if (cheat_state->killark.haha_friends[iSAMPID])
			{
					h = pD3DFont_sampStuff->DrawHeight() - 9;
					sprintf_s(buf, "WALLNER");
					pD3DFontFixedSmall->PrintShadow(g_playerTagInfo[iGTAID].tagPosition.fX, playerBaseY - h - 23.0f,
						D3DCOLOR_ARGB(255, 0, 255, 0), buf);
				}
				/*if (g_NewPatch.FriendID[iSAMPID] && g_NewPatch.FriendDogrulama[iSAMPID] && g_NewPatch.FriendName[iSAMPID] == getPlayerName(iSAMPID))
				{
					...
					fPlayerNameTagX = g_playerTagInfo[iGTAID].tagPosition.fX + ESP_tag_player_D3DBox_pixelOffsetX / 2 + 50.0f - pD3DFont_sampStuff->DrawLength(buf) / 2;
					pD3DFont_sampStuff->PrintShadow(g_playerTagInfo[iGTAID].tagPosition.fX, playerBaseY - h - 8.0f, D3DCOLOR_ARGB(255, 0, 255, 0), buf);
					sprintf_s(buf, " FRIEND");
					pD3DFontFixedSmall->PrintShadow(g_playerTagInfo[iGTAID].tagPosition.fX, playerBaseY - h - 23.0f,
						D3DCOLOR_ARGB(255, 0, 255, 0), buf);
				}
				else
				...
				{
				case PLAYER_STATE_ONFOOT:
					if (g_Players->pRemotePlayer[iSAMPID]->pPlayerData->onFootData.sCurrentAnimationID == 1701)
					{
						render->D3DBox(g_playerTagInfo[iGTAID].tagPosition.fX + 100.0f, playerBaseY - h + 30.0f, 50.0f, 10.0f, D3DCOLOR_ARGB(111, 0, 0, 0));
						pD3DFontFixedSmall->PrintShadow(g_playerTagInfo[iGTAID].tagPosition.fX + 100.0f, playerBaseY - h + 29.0f,
							D3DCOLOR_ARGB(130, 0xFF, 0x6A, 0), "Wounded");
						break;
					}
					else
					{
				...
				case PLAYER_STATE_DRIVER:
					render->D3DBox(g_playerTagInfo[iGTAID].tagPosition.fX + 100.0f, playerBaseY - h + 30.0f, 40.0f, 10.0f, D3DCOLOR_ARGB(111, 0, 0, 0));
					pD3DFontFixedSmall->PrintShadow(g_playerTagInfo[iGTAID].tagPosition.fX + 100.0f, playerBaseY - h + 29.0f,
						D3DCOLOR_ARGB(130, 0xFF, 0x6A, 0), "Driver");
					break;
				case PLAYER_STATE_PASSENGER:
					if (g_Players->pRemotePlayer[iSAMPID]->pPlayerData->iPassengerDriveBy)
					{
						render->D3DBox(g_playerTagInfo[iGTAID].tagPosition.fX + 100.0f, playerBaseY - h + 30.0f, 60.0f, 10.0f, D3DCOLOR_ARGB(111, 0, 0, 0));
						pD3DFontFixedSmall->PrintShadow(g_playerTagInfo[iGTAID].tagPosition.fX + 100.0f, playerBaseY - h + 29.0f,
							D3DCOLOR_ARGB(130, 0xFF, 0x6A, 0), "Drive by");
					}
					else
					{
						render->D3DBox(g_playerTagInfo[iGTAID].tagPosition.fX + 100.0f, playerBaseY - h + 30.0f, 65.0f, 10.0f, D3DCOLOR_ARGB(111, 0, 0, 0));
						pD3DFontFixedSmall->PrintShadow(g_playerTagInfo[iGTAID].tagPosition.fX + 100.0f, playerBaseY - h + 29.0f,
							D3DCOLOR_ARGB(130, 0xFF, 0x6A, 0), "Passenger");
						break;
					}
				case PLAYER_STATE_SPAWNED:
					...
					{
						render->D3DBox(g_playerTagInfo[iGTAID].tagPosition.fX + 100.0f, playerBaseY - h + 30.0f, 55.0f, 10.0f, D3DCOLOR_ARGB(111, 0, 0, 0));
						pD3DFontFixedSmall->PrintShadow(g_playerTagInfo[iGTAID].tagPosition.fX + 100.0f, playerBaseY - h + 29.0f,
							D3DCOLOR_ARGB(130, 0xFF, 0x6A, 0), "Spawned");
						break;
					}
				...
				if (g_Players->pRemotePlayer[iSAMPID]->pPlayerData->iAFKState == 2)
				{
					...
					render->D3DBox(fPlayerNameTagX + 62.0f, fAFKTextY - 10.0f, pD3DFont_sampStuff->DrawLength(buf) + 2.0f, 10.0f, D3DCOLOR_ARGB(111, 0, 0, 0));
					pD3DFontFixedSmall->PrintShadow(fPlayerNameTagX + 63.0f, fAFKTextY - 12.0f, D3DCOLOR_ARGB(130, 170, 170, 170), buf);
				}
				if (cheat_state->killark.jhonnyproject = true)
				{
					cheat_state->killark.killarkesp = false;
					cheat_state->killark.wallner = false;
					cheat_state->killark.astrong = false;
	...
	if (cheat_state->killark.astrong)
	{
		float h, playerBaseY;
		while (iter.pos < iter.end)
		{
			...
			updateScoreboardData();
			D3DCOLOR    color = D3DCOLOR_ARGB(112, 250, 15, 2);
			if (vh > 100.0f) // sal bi.
				vh = 100.0f;
			if (vh < 100.0f && vh > 82.0f)// kanka arkadaki bar siyah ya renkler ondan dolayı bozuldu
				color = D3DCOLOR_ARGB(112, 250, 15, 2);// %eskisi gibi olsun SİYAH TONLAMA. if koduna bakarsan ne demek istediğimi anlarsın.
			if (vh < 82.0f && vh > 68.0f)// Sen bırak bunu, ben kendi keyfime göre bi dizayn edeyim, dur.ç++
				color = D3DCOLOR_ARGB(112, 255, 42, 0); // Şimdi. seçkin ile kavgamız rapid'in bizi ilkel yollar ile ayırmayıp kavganın bitmesini beklemesi... :)) anani sikeyim seçkin arda xkraltr31
			if (vh < 68.0f && vh > 44.0f)
				color = D3DCOLOR_ARGB(112, 255, 111, 0);//XRGB Q kullansın, argb olayını anlamadı bu // OPAKlık kırmızı yeşil mavi
			if (vh < 44.0f && vh > 22.0f)
				color = D3DCOLOR_ARGB(112, 255, 162, 0);
			if (vh < 22.0f && vh > 0.0f)
				color = D3DCOLOR_ARGB(112, 255, 242, 0);

			render->D3DBox(g_playerTagInfo[iGTAID].tagPosition.fX + -45.0f,
				playerBaseY /* +set.D3DFeatures.playerTagYStuffX[0] */ + -30.0f, 100.0f, 35.0f, D3DCOLOR_ARGB(111, 0, 0, 0));
			render->D3DBox(g_playerTagInfo[iGTAID].tagPosition.fX + -43.8f,
				playerBaseY /* +set.D3DFeatures.playerTagYStuffX[1]*/ + -28.7f, vh - 2.0f, 10.0f, color);

			if (va > 100.0f)
				va = 100.0f;

			render->D3DBox(g_playerTagInfo[iGTAID].tagPosition.fX + -43.8f,
				playerBaseY + -17.7f, va - 2.0f, 10.0f, D3DCOLOR_ARGB(150, 118, 117, 118));

			if (ping > 500)
				ping = 0;
			render->D3DBox(g_playerTagInfo[iGTAID].tagPosition.fX + -43.8f,
				playerBaseY + -6.7f, ping / 3, 10.0f, D3DCOLOR_ARGB(255, 81, 143, 117));
			// yürüme cubu
			render->D3DBox(g_playerTagInfo[iGTAID].tagPosition.fX + 55.0f,
				playerBaseY + -5.0f, 70.0f, 10.0f, D3DCOLOR_ARGB(50, 0, 0, 0));
			// afk cubu
			...
			if (!g_Players)
			{
				...
				_snprintf_s(buf2, sizeof(buf2) - 1, "Armor: %d", (int)iterPed->GetArmor());
			...
			else
			{
				...
				_snprintf_s(buf, sizeof(buf) - 1, "Health: %d", (int)g_Players->pRemotePlayer[iSAMPID]->pPlayerData->fActorHealth);
				pD3DFontFixedSmall->PrintShadow(g_playerTagInfo[iGTAID].tagPosition.fX - 40.0f, playerBaseY - h + -18.0f,
					D3DCOLOR_ARGB(150, 247, 247, 247), buf);

				_snprintf_s(buf2, sizeof(buf2) - 1, "Armor: %d", (int)g_Players->pRemotePlayer[iSAMPID]->pPlayerData->fActorArmor);
				pD3DFontFixedSmall->PrintShadow(g_playerTagInfo[iGTAID].tagPosition.fX - 40.0f, playerBaseY - h + -7.0f,
					D3DCOLOR_ARGB(150, 247, 247, 247), buf2);

				updateScoreboardData();
				stRemotePlayer* pPlayer = g_SAMP->pPools->pPlayer->pRemotePlayer[iSAMPID];
				_snprintf_s(buf3, sizeof(buf3) - 1, "Ping: %d", pPlayer->iPing);
				pD3DFontFixedSmall->PrintShadow(g_playerTagInfo[iGTAID].tagPosition.fX - 40.0f, playerBaseY - h + 4.0f,
					D3DCOLOR_ARGB(150, 247, 247, 247), buf3);
				h = pD3DFont_sampStuff->DrawHeight() - 1;
				if (cheat_state->killark.haha_friends[iSAMPID])
					sprintf_s(buf, "{00FF00}%s (%d)", getPlayerName(iSAMPID), iSAMPID);
				else
					sprintf_s(buf, "%s (%d)", getPlayerName(iSAMPID));
				pD3DFont_sampStuff->PrintShadow(g_playerTagInfo[iGTAID].tagPosition.fX - 47.0f, playerBaseY - h + -40.0f,
					samp_color_get(iSAMPID, 0xDD000000), buf);

				float w = pD3DFont_sampStuff->DrawLength(buf) - 1;
				...
				if (g_Players->pRemotePlayer[iSAMPID]->pPlayerData->iAFKState == 2) // player is AFK
				{
					...
					sprintf_s(bufXD, "AFK");
					render->D3DBox(g_playerTagInfo[iGTAID].tagPosition.fX + 55.0f,
						playerBaseY + -30.0f, 30.0f, 10.0f, D3DCOLOR_ARGB(150, 0, 0, 0));
					pD3DFontFixedSmall->PrintShadow(g_playerTagInfo[iGTAID].tagPosition.fX + 58.0f, playerBaseY - h + -17.0f,
						D3DCOLOR_ARGB(150, 137, 135, 135), bufXD);
				...
				{
				case PLAYER_STATE_ONFOOT:
					sprintf_s(bufXD, "OnFoot"); // Tanımlara gerek yok, biliniyor hepsi zaten.
					color = D3DCOLOR_ARGB(150, 255, 255, 255); // Yeşil renk
					break;
				case PLAYER_STATE_DRIVER:
					sprintf_s(bufXD, "Driver");
					color = D3DCOLOR_ARGB(150, 255, 255, 255); // Kırmızı.
					break;
				case PLAYER_STATE_PASSENGER:
					sprintf_s(bufXD, "Passenger");
				...
				case PLAYER_STATE_WASTED: // Bu benim yaptığım ilave, oyuncu öldüğünde state wasted olur, ölü işte.
					sprintf_s(bufXD, "Wasted");
					color = D3DCOLOR_ARGB(150, 255, 255, 255); // Direk kırmızı renk.
					break;
				case PLAYER_STATE_SPAWNED:
					sprintf_s(bufXD, "Spawned");
					color = D3DCOLOR_ARGB(150, 255, 139, 0); // Direk kırmızı renk.
					break;
				case PLAYER_STATE_NONE:
					sprintf_s(bufXD, "None");
					color = D3DCOLOR_ARGB(150, 255, 139, 0); // Direk kırmızı renk.
					break;
				}

				if (bufXD)
					pD3DFontFixedSmall->PrintShadow(g_playerTagInfo[iGTAID].tagPosition.fX + 58.5f, playerBaseY - h + 7.0f,
						D3DCOLOR_ARGB(150, 255, 139, 0), bufXD);
				...
				for (int i = 0; i < 45; i++)
				{
					...
					if (getPlayerState(iSAMPID) == PLAYER_STATE_DRIVER)
					{
						if (weapon_list[i].id == g_Players->pRemotePlayer[iSAMPID]->pPlayerData->inCarData.byteCurrentWeapon)
						{
							...
							h = pD3DFont_sampStuff->DrawHeight() - 40.0f;//4
							_snprintf_s(buf222, sizeof(buf222) - 1, "%s ", weapon_list[i].name);


							pD3DFont_sampStuff->PrintShadow(g_playerTagInfo[iGTAID].tagPosition.fX + -30.0f, playerBaseY + 40 - h - 60,
								D3DCOLOR_ARGB(255, 255, 139, 0), buf222);
					...
					if (getPlayerState(iSAMPID) == PLAYER_STATE_ONFOOT)
					{
						if (weapon_list[i].id == g_Players->pRemotePlayer[iSAMPID]->pPlayerData->onFootData.byteCurrentWeapon)
						{
							...
							pD3DFont_sampStuff->PrintShadow(g_playerTagInfo[iGTAID].tagPosition.fX + -30.0f, playerBaseY + 40 - h - 45,
								D3DCOLOR_ARGB(255, 255, 139, 0), buf222);
					...
					if (getPlayerState(iSAMPID) == PLAYER_STATE_PASSENGER)
					{
						if (weapon_list[i].id == g_Players->pRemotePlayer[iSAMPID]->pPlayerData->passengerData.byteCurrentWeapon)
						{
							...
							if (g_Players->pRemotePlayer[iSAMPID]->pPlayerData->iPassengerDriveBy)
							{
								...
								h = pD3DFont_sampStuff->DrawHeight() - 25.0f;//1
								_snprintf_s(buf222, sizeof(buf222) - 1, "%s (DRIVE-BY) ", weapon_list[i].name);
								pD3DFont_sampStuff->PrintShadow(g_playerTagInfo[iGTAID].tagPosition.fX + -30.0f, playerBaseY + 40 - h - 45,
									D3DCOLOR_ARGB(255, 255, 139, 0), buf222);
								break;
				...
				if (cheat_state->killark.astrong = true)
				{
					cheat_state->killark.killarkesp = false;
					cheat_state->killark.wallner = false;
					cheat_state->killark.jhonnyproject = false;
	...
	if (cheat_state->killark.veipzesp)
	{
		float h, playerBaseY;
		while (iter.pos < iter.end)
		{
			...
			updateScoreboardData();
			D3DCOLOR    color = D3DCOLOR_ARGB(145, 70, 255, 255);
			if (vh > 100.0f)
				vh = 100.0f;
			if (vh < 100.0f && vh > 70.0f)
				color = D3DCOLOR_ARGB(145, 70, 255, 255);
			if (vh < 70.0f && vh > 40.0f)
				color = D3DCOLOR_ARGB(186, 14, 181, 255);
			if (vh < 40.0f && vh > 00.0f)
				color = D3DCOLOR_ARGB(216, 116, 8, 255);

			render->D3DBox(g_playerTagInfo[iGTAID].tagPosition.fX + -45.0f,
				playerBaseY + -30.0f, 100.0f, 26.0f, D3DCOLOR_ARGB(111, 0, 0, 0));
			render->D3DBox(g_playerTagInfo[iGTAID].tagPosition.fX + -43.8f,
				playerBaseY + -28.7f, vh - 2.0f, 10.0f, color);

			h = pD3DFontFixedSmall->DrawHeight() + 1;

			if (!g_Players)
			{
				_snprintf_s(buf, sizeof(buf) - 1, "           %d", (int)iterPed->GetHealth());
				pD3DFontFixedSmall->PrintShadow(g_playerTagInfo[iGTAID].tagPosition.fX + 8.0f, playerBaseY - h + 10.0f,
					D3DCOLOR_ARGB(130, 0xFF, 0x6A, 0), buf);

				_snprintf_s(buf2, sizeof(buf2) - 1, "           %d", (int)iterPed->GetArmor());
				pD3DFontFixedSmall->PrintShadow(g_playerTagInfo[iGTAID].tagPosition.fX + 8.0f, playerBaseY - h + 25.0f,
					D3DCOLOR_ARGB(130, 0xFF, 0x6A, 0), buf2);
			}
			else
			{
				...
				_snprintf_s(buf, sizeof(buf) - 1, "           %d", (int)g_Players->pRemotePlayer[iSAMPID]->pPlayerData->fActorHealth);
				pD3DFontFixedSmall->PrintShadow(g_playerTagInfo[iGTAID].tagPosition.fX - 40.0f, playerBaseY - h + -18.0f,
					D3DCOLOR_ARGB(150, 255, 139, 0), buf);

				_snprintf_s(buf2, sizeof(buf2) - 1, "           %d", (int)g_Players->pRemotePlayer[iSAMPID]->pPlayerData->fActorArmor);
				pD3DFontFixedSmall->PrintShadow(g_playerTagInfo[iGTAID].tagPosition.fX - 40.0f, playerBaseY - h + -7.0f,
					D3DCOLOR_ARGB(150, 255, 139, 0), buf2);

				updateScoreboardData();
				if (cheat_state->killark.haha_friends[iSAMPID])
					sprintf_s(buf, "{00FF00}%s (%d)", getPlayerName(iSAMPID), iSAMPID);
				else if (g_Players->pRemotePlayer[iSAMPID]->pPlayerData->onFootData.sCurrentAnimationID == 1701)
					sprintf_s(buf, "{FF0000}%s (%d - %d)", getPlayerName(iSAMPID), iSAMPID, g_Players->pRemotePlayer[iSAMPID]->iScore);
				else
					sprintf_s(buf, "%s (%d - %d)", getPlayerName(iSAMPID), iSAMPID, g_Players->pRemotePlayer[iSAMPID]->iScore);
				pD3DFont_sampStuff->PrintShadow(g_playerTagInfo[iGTAID].tagPosition.fX - 47.0f, playerBaseY - h + -40.0f,
					samp_color_get(iSAMPID, 0xDD000000), buf);
			...
			if (g_Players->pRemotePlayer[iSAMPID]->pPlayerData->iAFKState == 2)
			{
				...
				case PLAYER_STATE_ONFOOT:
					sprintf_s(bufXD, "OnFoot"); // Tanımlara gerek yok, biliniyor hepsi zaten.
					color = D3DCOLOR_ARGB(150, 255, 139, 0); // Yeşil renk
					break;
				case PLAYER_STATE_DRIVER:
					sprintf_s(bufXD, "Driver");
					color = D3DCOLOR_ARGB(150, 255, 139, 0); // Kırmızı.
					break;
				case PLAYER_STATE_PASSENGER:
					sprintf_s(bufXD, "Passenger");
					color = D3DCOLOR_ARGB(150, 255, 139, 0);
					break;
				case PLAYER_STATE_WASTED: // Bu benim yaptığım ilave, oyuncu öldüğünde state wasted olur, ölü işte.
					sprintf_s(bufXD, "Wasted");
					color = D3DCOLOR_ARGB(150, 255, 139, 0); // Direk kırmızı renk.
					...
					if (getPlayerState(iSAMPID) == PLAYER_STATE_PASSENGER)
					{
						if (weapon_list[i].id == g_Players->pRemotePlayer[iSAMPID]->pPlayerData->passengerData.byteCurrentWeapon)
						{
							...
							if (g_Players->pRemotePlayer[iSAMPID]->pPlayerData->iPassengerDriveBy)
							{
								...
								h = pD3DFont_sampStuff->DrawHeight() - 25.0f;//1
								_snprintf_s(buf222, sizeof(buf222) - 1, "%s (DRIVE-BY) ", weapon_list[i].name);
								pD3DFont_sampStuff->PrintShadow(g_playerTagInfo[iGTAID].tagPosition.fX + -33.0f, playerBaseY + 40 - h - 45,
									D3DCOLOR_ARGB(255, 255, 139, 0), buf222);
								break;

							}
							if (cheat_state->killark.veipzesp = true)
							{
								cheat_state->killark.killarkesp = false;
								cheat_state->killark.wallner = false;
								cheat_state->killark.jhonnyproject = false;
...
void renderVehicleTags(void)
{
	...
	while (iter.pos < iter.end)
	{
		...
		ourPosMinusIter = ourPosition - iterPosition;
		if (ourPosMinusIter.Length() > set.vehicle_tags_d1st)
			continue;
...
void RenderTeleportTexts(void)
{
	...
	if (self != NULL)
	{
		for (i = 0; i < TELEPORT_MAX; i++)
		{
			...
			if (vect3_near_zero(pos))
				continue;
			if (vect3_dist(pos, &self->base.matrix[4 * 3]) > set.player_tags_d1st)
				continue;
...
void RenderObjectTexts(void)
{
	...
	for (i = 0; i < SAMP_MAX_OBJECTS; i++)
	{
		...
		if (vect3_near_zero(pos))
			continue;
		if (vect3_dist(pos, &self->base.matrix[4 * 3]) > set.object_tags_d1st)
			continue;
...
void renderChat(void)
{
	...
	{
		if (set.d3dtext_ch1t_lines <= 0)
			set.d3dtext_ch1t_lines = 10;
		if (set.d3dtext_ch1t_lines >= 100)
			set.d3dtext_ch1t_lines = 100;
		...
		float	pw;
		int		i = 99;
		int		iLinesForLoop = (i - set.d3dtext_ch1t_lines);
...
void renderSAMP(void)
{
	...
	if (g_SAMP != NULL)
	{
		...
		if (!a)
		{
			if (set.player_tags_d1st > set.line_of_sight_dist)
				memcpy_safe((void*)(g_dwSAMP_Addr + SAMP_DRAWING_DISTANCE), &set.player_tags_d1st, sizeof(float));
			else
...
#define HUD_TEXT_TGL( x, color, text ) \
	...
	uint32_t    color_text = D3DCOLOR_XRGB(255, 255, 255);
	uint32_t    color_enabled = D3DCOLOR_XRGB(217, 54, 4);
	uint32_t    color_disabled = D3DCOLOR_XRGB(255, 255, 255);
	...
	if (isBeginRenderWIN)
	{
		...
		if (set.d3dtext_hud)
		{
			if (cheat_panic() || cheat_state->state == CHEAT_STATE_NONE)
			{
				...
				if (!set.flickering_problem)
				{
					if (iIsSAMPSupported)
					{
						...
						_snprintf_s(buf, sizeof(buf) - 1, "                         PROJECT KILLARK");
						HUD_TEXT(x, D3DCOLOR_XRGB(152, 135, 255), buf);
					}
					else
					{
						uint32_t	bar_color = D3DCOLOR_XRGB(hud_bar->alpha, hud_bar->red, hud_bar->green,
							hud_bar->blue);
						render->D3DBoxi((int)x - 1,
							(int)(pPresentParam.BackBufferHeight - 1) - (int)pD3DFont->DrawHeight() - 3,
							(int)(pPresentParam.BackBufferWidth + 14), 22, bar_color, NULL);
						HUD_TEXT(x, D3DCOLOR_XRGB(152, 135, 255), "                        PROJECT KILLARK");
					}
				}
			...
			if (cheat_state->state == CHEAT_STATE_VEHICLE)
			{
				...
				HUD_TEXT_TGL(x, cheat_state->killark.silentaim ? color_enabled : color_disabled, "Silent");

				HUD_TEXT_TGL(x, cheat_state->killark.invisible ? color_enabled : color_disabled, "Invisible");

				HUD_TEXT_TGL(x, cheat_state->killark.afk ? color_enabled : color_disabled, "Afk");

				HUD_TEXT_TGL(x, cheat_state->killark.noreload ? color_enabled : color_disabled, "No Reload");

				HUD_TEXT_TGL(x, cheat_state->killark.box ? color_enabled : color_disabled, "Box");

				HUD_TEXT_TGL(x, cheat_state->killark.slapper ? color_enabled : color_disabled, "Slapper");
		...
		if (cheat_state->debug_enabled)
			RenderDebug();
		if (cheat_state->render_vehicle_t4gs)
			renderVehicleTags();
