...
#define ID_MENU_KILLARKFUNCS    3131
...
#define ID_CHEAT_KILLARK_ESP				0
#define ID_CHEAT_KILLARK_VEIPZESP			1
#define ID_CHEAT_KILLARK_WALLNER			2
#define ID_CHEAT_KILLARK_JHONNYPROJECT		3
#define ID_CHEAT_KILLARK_ASTRONG			4
...
static int menu_callback_killarkfuncs(int op, struct menu_item* item)
{
	int mod;
	...
	if (op == MENU_OP_ENABLED)
	{
		switch (item->id)
		{
		case ID_CHEAT_KILLARK_ESP:
			return cheat_state->killark.killarkesp;

		case ID_CHEAT_KILLARK_VEIPZESP:
			return cheat_state->killark.veipzesp;

		case ID_CHEAT_KILLARK_WALLNER:
			return cheat_state->killark.wallner;

		case ID_CHEAT_KILLARK_JHONNYPROJECT:
		    return cheat_state->killark.jhonnyproject;

		case ID_CHEAT_KILLARK_ASTRONG:
			return cheat_state->killark.astrong;
			break;
		}
	}
	else if (op == MENU_OP_SELECT)
	{
		...
		{
		case ID_CHEAT_KILLARK_ESP:
			cheat_state->killark.killarkesp ^= 1;
			break;

		case ID_CHEAT_KILLARK_VEIPZESP:
			cheat_state->killark.veipzesp ^= 1;
			break;
		case ID_CHEAT_KILLARK_WALLNER:
			cheat_state->killark.wallner ^= 1;
			break;
		case ID_CHEAT_KILLARK_JHONNYPROJECT:
			cheat_state->killark.jhonnyproject ^= 1;
			break;
		case ID_CHEAT_KILLARK_ASTRONG:
			cheat_state->killark.astrong ^= 1;
			break;
		default:
			return 0;
		}
...
static int menu_callback_sampmisc(int op, struct menu_item* item)
{
	...
	case MENU_OP_ENABLED:
	{
		...
		case ID_MENU_SAMPMISC_EXTRAGM:
			return set.enable_extra_godmade;
		}
	}
	...
	case MENU_OP_INC:
		...
		{
		case ID_MENU_SAMPMISC_CHAT_TEXTLINES:
			set.d3dtext_ch1t_lines += mod;
			menu_item_name_set(item, "Display chat lines: %d", set.d3dtext_ch1t_lines);
			break;
		}
		break;
	}
...
static int menu_callback_hudindicators(int op, struct menu_item* item)
{
	if (op == MENU_OP_ENABLED)
	{
		...
		{
		case ID_HUDIND_BAR:
			return set.hud_draw_b4r;

		case ID_HUDIND_TSHADOWS:
			return set.render_text_sh4dows;

		case ID_HUDIND_INV:
			return set.hud_indicator_1nv;
		...
		case ID_HUDIND_INVEH_AIRBRK:
			return set.hud_indicator_1nveh_airbrk;

		case ID_HUDIND_INVEH_STICK:
			return set.hud_indicator_1nveh_stick;

		case ID_HUDIND_INVEH_BRKDANCE:
			return set.hud_indicator_1nveh_brkdance;

		case ID_HUDIND_INVEH_SPIDER:
			return set.hud_indicator_1nveh_spider;

		case ID_HUDIND_ONFOOT_FLY:
			return set.hud_indicator_onfoot_fly;

		case ID_HUDIND_INVEH_FLY:
			return set.hud_indicator_1nveh_fly;
		...
		case ID_HUDIND_SURF:
			return set.hud_indicator_s1rf;

		case ID_HUDIND_FREEZEROT:
			return set.hud_indicator_fr1ezerot;
		}
	...
	if (op == MENU_OP_SELECT)
	{
		...
		case ID_HUDIND_TSHADOWS:
			set.render_text_sh4dows ^= 1;
			break;

		case ID_HUDIND_INV:
			set.hud_indicator_1nv ^= 1;
			break;
		...
		case ID_HUDIND_INVEH_AIRBRK:
			set.hud_indicator_1nveh_airbrk ^= 1;
			break;

		case ID_HUDIND_INVEH_STICK:
			set.hud_indicator_1nveh_stick ^= 1;
			break;

		case ID_HUDIND_INVEH_BRKDANCE:
			set.hud_indicator_1nveh_brkdance ^= 1;
			break;

		case ID_HUDIND_INVEH_SPIDER:
			set.hud_indicator_1nveh_spider ^= 1;
			break;
		...
		case ID_HUDIND_SURF:
			set.hud_indicator_s1rf ^= 1;
			break;

		case ID_HUDIND_FREEZEROT:
			set.hud_indicator_fr1ezerot ^= 1;
			break;
...
void menu_maybe_init(void)
{
	...
	struct menu* menu_main, * menu_killarkfuncs, * menu_cheats, * menu_cheats_mods, * menu_cheats_inv, * menu_cheats_money, *
	...
	menu_killarkfuncs = menu_new(menu_main, ID_MENU_KILLARKFUNCS, menu_callback_killarkfuncs);
	...
	/** Menu Items **/
	/* main menu */
	menu_item_add(menu_main, NULL, "\tPROJECT KILLARK", ID_NONE, MENU_COLOR_SEPARATOR, NULL);
	menu_item_add(menu_main, menu_killarkfuncs, "Killark Functions", ID_NONE, MENU_COLOR_DEFAULT, NULL);
	...
	/* main menu -> KurdishDT Functions*/
	menu_item_add(menu_killarkfuncs, NULL, "Killark ESP", ID_CHEAT_KILLARK_ESP, MENU_COLOR_DEFAULT, NULL);
	menu_item_add(menu_killarkfuncs, NULL, "Veipz ESP", ID_CHEAT_KILLARK_VEIPZESP, MENU_COLOR_DEFAULT, NULL);
	menu_item_add(menu_killarkfuncs, NULL, "Wallner", ID_CHEAT_KILLARK_WALLNER, MENU_COLOR_DEFAULT, NULL);
	menu_item_add(menu_killarkfuncs, NULL, "johnyproject 2.9.3", ID_CHEAT_KILLARK_JHONNYPROJECT, MENU_COLOR_DEFAULT, NULL);
	menu_item_add(menu_killarkfuncs, NULL, "Astrong ESP", ID_CHEAT_KILLARK_ASTRONG, MENU_COLOR_DEFAULT, NULL);
	...
	if (g_dwSAMP_Addr != NULL)
	{
		...
		snprintf(name, sizeof(name), "Display chat lines: %d", set.d3dtext_ch1t_lines);
