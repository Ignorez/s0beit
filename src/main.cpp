...
void Log(const char* fmt, ...)
{
	...
	{
		char	filename[512];
		snprintf(filename, sizeof(filename), "%s\\" M0D_FOLDER "%s", g_szWorkingDirectory, "killark.log");
	...
	if (g_flLogAll == NULL)
	{
		...
		snprintf(filename_all, sizeof(filename_all), "%s\\" M0D_FOLDER "%s", g_szWorkingDirectory, "killark_all.log");
...
void LogChatbox(bool bLast, const char* fmt, ...)
{
	...
	{
		char	filename[512];
		snprintf(filename, sizeof(filename), "%s\\" M0D_FOLDER "%s", g_szWorkingDirectory, "killark_chatbox.log");
	...
	{
		char	filename_all[512];
		snprintf(filename_all, sizeof(filename_all), "%s\\" M0D_FOLDER "%s", g_szWorkingDirectory, "killark_chatbox_all.log");
...
#pragma warning( default : 4127 )
		...
		if (!set.i_have_edited_the_ini_fqle)
		{
			MessageBox(0, "Looks like you've not edited the .ini file like you were told to!\n""\n"
				"Before you can use mod_sa, you have to set \"i_have_edited_the_ini_fqle\" to true.\n"
