	...
	if ( KEYCOMBO_PRESSED(set.key_autoaim_toggle) )
	{
		...
		if ( set.use_gta_autoaim )
		{
			if ( cheat_state->actor.autoaim == 1 )
			{
				...
				*(char *)0x00B6EC2E = 1;
				*(char *)0x00BA6818 = 0;
				patcher_4nstall( &patch_gta_auto_aim );
			}
			else
			{
...
void cheat_handle_actor_4ir_brake ( struct actor_info *info, double time_diff )
{
	traceLastFunc( "cheat_handle_actor_4ir_brake()" );
...
void cheat_handle_SpiderFeet ( struct actor_info *ainfo, double time_diff )
{
	...
	if ( KEY_PRESSED(set.key_spiderfeet) )
	{
		...
		{
			//patcher_4nstall( &patch_actor_SpiderFeetCollisionTransform );
		}
...
// keys/buttons input
			...
			if ( !cheat_state->actor.fly_active )
			{
				...
				if (!patch_RemoveFlyUpSpeedLimit.installed)
				{
					patcher_4nstall(&patch_RemoveFlyUpSpeedLimit);
				}
				// install fly soft limiters patch
				if (!patch_RemoveFlyWindSpeedLimit.installed)
				{
					patcher_4nstall(&patch_RemoveFlyWindSpeedLimit);
				}
...
{
	if (KEYCOMBO_PRESSED(set.key_kick))
	{
		disconnect(0);
		restartGame();
	}
