...
public:
	...
	void SendGiveDamage(uint16_t uiPlayerID, float fDamageAmountToSend, int iWeaponID, int iBodyPart);
	void SendSpawn(void);
	void SendFakeOnfootSyncData_ForInvisible();
	void SendFakeDriverSyncData(WORD vehicleid, float fPos[3], float HealthCar, float Speed[3], WORD key, WORD trailer = -1);
